...
def display_watchlist(watchlist_name):
    ...
    with col1:
        new_ticker = st.text_input(f"Ajouter un ticker", key=f"input_{watchlist_name}")
    with col2:
        if st.button("Ajouter", key=f"btn_add_{watchlist_name}") and new_ticker:
    ...
    for i, sym in enumerate(st.session_state[watchlist_name]):
        ...
        with col_info:
            ...
            """, unsafe_allow_html=True)
        with col_del:
            if st.button("✕", key=f"del_{watchlist_name}_{i}"):
                to_delete = i

    st.markdown('</div>', unsafe_allow_html=True)

    if to_delete is not None:
        st.session_state[watchlist_name].pop(to_delete)
        st.rerun()
...
if selected_section == "Watchlist1":
    st.title("Watchlist 1 - Style Google Finance")
    display_watchlist("watchlist1")

elif selected_section == "Watchlist2":
    st.title("Watchlist 2 - Style Google Finance")
    ...
    with col1:
        ticker_impot = st.text_input("Ticker", value="AAPL", key="ticker_impot")
    ...
    if fichier_csv is not None:
        try:
            ...
            if not all(col in df.columns for col in colonnes_attendues):
                display_message("error", f"Le fichier doit contenir: {', '.join(colonnes_attendues)}")
            else:
    ...
    # Graphique plus-values par pays
    ...
    if st.session_state.transactions:
        ...
        pays_plus_rentable = df.groupby("Pays").apply(
            lambda d: ((d["Prix Vente"] - d["Prix Achat"]) * d["Quantité"]).sum()
        ).idxmax()

        col1, col2, col3, col4 = st.columns(4)
        col1.metric("Plus-value totale", f"{total_pv:.2f} €")
        col2.metric("Impôt total estimé", f"{total_impot:.2f} €")
        col3.metric("Nb transactions", nb_transactions)
        col4.metric("Pays le plus rentable", pays_plus_rentable)
    else:
        display_message("info", "Aucune transaction enregistrée.")
    ...
    if st.button("Simuler la vente"):
        try:
            ...
            if info.empty:
                ...
                taux_map = {"France": 0.3, "UK": 0.1, "Allemagne": 0.25, "Canada": 0.15, "Autre": 0.3}
                if pays_sim == "USA":
                    taux = 0.15 if duree_sim >= 1 else 0.3
                else:
                    taux = taux_map.get(pays_sim, 0.3)
                ...
                col2.metric("Plus-value", f"{plus_value:.2f} €")
                col3.metric("Impôt estimé", f"{impot:.2f} €")
                col4.metric("Net après impôt", f"{net:.2f} €")
        except Exception as e:
